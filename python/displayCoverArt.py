...
def main() -> int:
    query = {'lat' : '12.345678', 'lon' : '9.123456', 'units' : 'metric', 'exclude' : 'minutely,alerts', 'appid' : 'xxx'}
    ...
    def getIcon(icon_name):
        ...
        return Image.alpha_composite(black_img,Image.open(os.path.join(dir,'../wheather_icons/' + icon_path)).convert('RGBA')).convert('RGB')
      ...
      while True:
        try:
          ...
          if imageURL is not None:
            ...
            #icon = Image.open(os.path.join(dir,'../wheather_icons/rain.png')).paste(black_img).convert('RGB')
            #icon = Image.alpha_composite(black_img,Image.open(os.path.join(dir,'../wheather_icons/rain.png')).convert('RGBA')).convert('RGB')
            ...
            graphics.DrawText(canvas, font, 10, 16, graphics.Color((int(secs * 4.25) + 125) % 255, 255 - int(secs * 4.25) % 255, int(secs * 4.25) % 255), secs_text)
            canvas.SetImage(main_icon, length, 0)
            graphics.DrawText(canvas, font_small, length + 18, 8, text_color, str(response_json['current']['temp']) + '°')
            graphics.DrawText(canvas, font_small, length + 18, 16, text_color, str(response_json['current']['humidity']) + '%')
            for i in range(1, 5):
